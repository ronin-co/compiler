...
The interface of creating new `Transaction` instances (thereby creating new transactions) was chosen in order to define the smallest unit of workload that the compiler can operate on.

Just like in the database, a transaction defines an atomic operation in which a list of queries is executed serially, and where each query can rely on the changes made by a previous one. In order to facilitate this, a programmatic interface that clarifies the accumulation of state is required (class instances).
