...
export const InstructionsSchema = z.object({
	with: z.union(
		[
			...
			z
				.array(
					WithInstructionSchema.refine(
						(value) => Object.keys(value).length > 0,
						{
							message:
								"A `with` instruction must reference at least one field.",
						},
					),
				)
		...
		],
		{
			errorMap: () => ({
				message:
					"A `with` instruction must either be an object or an array of objects.",
			}),
		},
	),
	...
	selecting: z.array(
		z.string({
			invalid_type_error:
				"The `selecting` instruction must be an array of strings.",
		}),
	),
	...
	limitedTo: z
		...
		.max(
			1000,
			"The `limitedTo` instruction must be less than or equal to 1000.",
		),
...
export const QuerySchema = z
	.object({
		[QueryTypeEnum.Enum.count]: z.record(
			z.string(),
			CountInstructionsSchema.nullable(),
		),
		...
		[QueryTypeEnum.Enum.get]: z.record(
			z.string(),
			GetInstructionsSchema.nullable(),
		),
