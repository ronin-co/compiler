 ...
 */
const composeMetaSchemaSlug = (suffix: string) =>
	convertToCamelCase(`ronin_${suffix}`);
 ...
 */
export const composeAssociationSchemaSlug = (
	schema: PublicSchema,
	field: SchemaField,
...
export const getFieldFromSchema = (
	...
	if (fieldPath.includes(".")) {
		schemaField = schemaFields.find(
			(field) => field.slug === fieldPath.split(".")[0],
		);
 ...
 */
export const addDefaultSchemaFields = (
	schema: PublicSchema,
	isNew: boolean,
): Schema => {
 ...
 */
const mappedInstructions: Partial<
	Record<QueryType, QueryInstructionTypeClean>
> = {
	create: "to",
	set: "with",
	drop: "with",
...
export const addSchemaQueries = (
	...
	const queryInstructions = queryInstructionsRaw as Instructions &
		SetInstructions;
	...
	const instructionName = mappedInstructions[
		queryType
	] as QueryInstructionTypeClean;
	...
	const slug: string | null =
		instructionList?.slug?.being || instructionList?.slug;
	...
	if (kind === "triggers") {
		...
		if (queryType === "create") {
			...
			if (
				filterQuery ||
				effectQueries.some((query) =>
					findInObject(query, RONIN_SCHEMA_SYMBOLS.FIELD),
				)
			) {
	...
	if (kind === "schemas") {
		...
		if (queryType === "create" || queryType === "set") {
			...
			const schemaWithShortcuts = addDefaultSchemaShortcuts(
				schemas,
				schemaWithFields,
			);
